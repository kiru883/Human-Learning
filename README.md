...
![](https://github.com/kiru883/Courses/blob/master/CV%20notes/images/seq2seq.png)
  ...
  В торчевском туториале при обучении seq2seq'а используется техника **force learning** при которой в декодер части при предсказании текущего токена, в качестве предыдущего используются не предсказанный токен, а трушный.
...
Ссылки
  ...
  - https://pytorch.org/tutorials/intermediate/seq2seq_translation_tutorial.html - seq2seq торчевская реализация переводчика англ.-франц., attention часть смотреть не обязательно, достаточно encoder, decoder, training
